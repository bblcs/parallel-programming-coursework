...
\newcommand{\thisLectureNum}{\foundationsNum}

\title[]{Lecture \thisLectureNum: \foundationsTopic}
...
Observations:
\begin{itemize}
  \pause \item it takes \textbf{several mathematically-intensive sessions} to go from the definition to really interesting theorems
  \pause \item concurrency is more ''inherently-complicated'' so it takes \textbf{even more time} to rigorously prove non-trivial properties
\end{itemize}
...
\begin{frame}[t]{Supplementary materials}
...
\pause
It looks like Java and feels like Java but
\begin{itemize}
  \pause \item algorithms from the slides \textbf{will not work} as expected if run on the JVM  
\end{itemize}
...
\pause
We are going to do some real Computer Science
\begin{itemize}
  \pause \item it should work from mathematical point of view ;)
\end{itemize}

\pause
Theoretical intuition and insights will help us to understand 
\begin{itemize}
  \pause \item how to implement efficient and correct algorithms
  \pause \item using existing hardware
\end{itemize}
...
\begin{itemize}
  \item Assign constant to local variable
  \item Invoke method
...
\begin{frame}[t, fragile]{Meta-language alert}

\textbf{Important}: we are using quite familiar notation
\begin{itemize}
  \item Undivisible events (elementary operations)
  \item Ordered on single timeline
\end{itemize}

\end{frame}

\questiontime{Totally ordered events on a single timeline. Have we encountered similar concept before?}

\begin{frame}[t,fragile,noframenumbering]{Meta-language alert}
...
This is \textbf{not} interleaving model!

\begin{itemize}
  \pause \item We guarantee order of \textbf{events} only 
  \pause \item Assignment to shared memory location could be represented as \textbf{two} events (interval)
  \begin{itemize}
    \item Thread A \texttt{starts assignment of value V to location L}
    \item Thread A \texttt{finishes assignment of value V to location L}
  \end{itemize}
  \pause \item Overlapping intervals happen ''simultaneously'' (''in parallel'', ''independent'')
\end{itemize}

\pause
We are developing a formal language to express different concurrency models. 

\pause
Interleaving model would be the simplest approach, ''one of many''.
...
\begin{frame}{Homework: precedence}

{\small\url{https://github.com/Svazars/parallel-programming/tree/main/hw/block2/7.1}}
\begin{homeworkmail}{Task 7.1}
    Prove that precedence is irreflexive, antisymmetric, transitive partial order.
\end{homeworkmail}

\end{frame}

\begin{frame}{Jargon alert}

\textbf{Happens-before} is an overloaded concept
\begin{itemize}
  \pause \item Language memory model of Java language
  \pause \item Language memory model of C++ language (not the same!)
  \pause \item Mathematical notion used in ''The Art of Multiprocessor Programming'' to teach students
  \pause \item Formalisms used in academic papers to express hardware memory models
  \pause \item ...
\end{itemize}

\pause 
In our course we \textbf{will not} use term ''happens-before'' to avoid confusion

\begin{itemize}
  \pause \item Precedence -- partial order on intervals consisting of elementary events
  \pause \item Precedence + additional rules = something more advanced
\end{itemize}

\pause
Please wait until Lecture~\langMMNum.
...
\begin{frame}[t]{Mutual exclusion}
...
\begin{frame}[fragile]{Homework: Filter Lock}

{\small\url{https://github.com/Svazars/parallel-programming/tree/main/hw/block2/7.2}}
\begin{homeworkmail}{Task 7.2}
    \begin{itemize}
      \item Prove that Filter Lock algorithm satisfies mutual exclusion.
      \item Prove that Filter Lock algorithm is starvation-free.
    \end{itemize}
\end{homeworkmail}

Supplementary materials:
\begin{itemize}
    \item Slides 77-93 from \texttt{chapter\_02.ppt}
    \item Section 2.4 ''The Filter Lock'' (pages 28 - 31) from ''The Art of Multiprocessor Programming''
\end{itemize}

\end{frame}


\begin{frame}[t,fragile]{Filter Lock}
...
\begin{quote}
  Theorem 2.8.1. Any \texttt{Lock} algorithm that, by reading and writing memory, solves deadlock-free mutual exclusion for three threads must use at least three distinct memory locations.
\end{quote}

\pause

We will not prove this theorem because of time limits.

\pause

Key insights for keen readers (pages 37-40):
\begin{itemize}
  \item Covering state
\end{itemize}
...
Generalized theorem:
\begin{quote}
  $N$-thread deadlock-free mutual exclusion requires $N$ distinct locations.
\end{quote}
...
\begin{frame}[fragile]{Fairness and Bakery algorithm}

{\small\url{https://github.com/Svazars/parallel-programming/tree/main/hw/block2/7.3}}
\begin{homeworkmail}{Task 7.3}
    \begin{itemize}
      \item Prove that Bakery Algorithm is deadlock-free
      \item Prove that Bakery Algorithm is first-come-first-served
      \item Prove that Bakery Algorithm satisfies mutual exclusion
    \end{itemize}
\end{homeworkmail}
...
%Supplementary materials:
%\begin{itemize}
    %\item slides 95-118 from \texttt{chapter\_02.ppt}
    %\item Section 2.5 ''Fairness'' and Section 2.6 ''Lamport`s Bakery Algorithm'' (pages 31 - 33)
%\end{itemize}
...
%\begin{frame}[fragile]{Before the final test}
%
...
\begin{frame}[t]{Sequential objects and specifications}

Sequential object:
\begin{itemize}
  \item State -- set of fields
  \item Methods -- the only way to change state  
\end{itemize}
...
\begin{frame}[t]{Sequential specifications}
...
\begin{itemize}
  \pause \item Remember to check if your approach is compositional
\end{itemize}
...
\begin{itemize}
  \item There will be no questions related to this topic on final exam
  \item No homework related to this questions
  \item I would be glad to answer your questions if you brave enough to dive in 
\end{itemize}

Information for keen readers:
\begin{itemize}
  \item Slides 95-176 from \texttt{chapter\_03.ppt}
  \item Chapter 3 ''Concurrent Objects'' up to section 3.6 ''Formal Definitions'' (pages 45 - 58)
\end{itemize}
...
\begin{frame}{Summary: homework}
    \texttt{Lecture 7, Tasks 7.*:} {\tiny\url{https://github.com/Svazars/parallel-programming/blob/main/hw/block2}}
\end{frame}
