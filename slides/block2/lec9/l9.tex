...
\newcommand{\thisLectureNum}{\atomicsNum}

\title[]{Lecture \thisLectureNum: \atomicsTopic}
\subtitle[]{\atomicsKey}
...
Concurrent objects
\begin{itemize}
  \item Sequential object, Sequential specification, Linearizability, Linearization points
\end{itemize}
...
\begin{itemize}
  \item Single write/multi read \textcolor{green}{OK}
  \item Multi write/multi read \textcolor{red}{impossible}
\end{itemize}

\end{frame}

\begin{frame}[t]{Consensus numbers measure synchronization power}
...
\pause \textbf{Reminder}: Task 2.4, NegativeSplitCounter
\end{frame}


\begin{frame}[t,fragile]{Read-modify-write operations not always are linearization points}

{\small\url{https://github.com/Svazars/parallel-programming/tree/main/hw/block2/9.1}}
\begin{homeworkmail}{Task 9.1}
    \begin{itemize}      
      \item \texttt{HWQueue.enq} consists of two instructions
      \item Prove that first instruction (\texttt{getAndIncrement}) is not a linearization point in some valid executions
      \item Prove that second instruction (\texttt{set}) is not a linearization point in some valid executions
      \item Consider provided counterexamples: do they prove that \texttt{HWQueue} is not linearizable?
    \end{itemize}
\end{homeworkmail}
\end{frame}
...
\begin{itemize}  
  ...
  \pause
  \item Read-modify-write operations not necessarily are linearization points
\end{itemize}
...
{\small\url{https://github.com/Svazars/parallel-programming/tree/main/hw/block2/9.2}}
\begin{homeworkmail}{Task 9.2}
    \begin{itemize}      
      \item Replace \texttt{AtomicBoolean} with \texttt{AtomicStampedReference(Thread.currentThread, counter)} and make TAS lock reentrant. 
      \item Provide at least 3 tests written in JCStress.      
    \end{itemize}
\end{homeworkmail}
...
{\small\url{https://github.com/Svazars/parallel-programming/tree/main/hw/block2/9.3}}
\begin{homeworkmail}{Task 9.3}
    Measure performance of \texttt{TASLock}, \texttt{TATASLock}, \texttt{ExpBackoffLock}
    \begin{itemize}
...
\begin{frame}[t,fragile]{Lock-free stack: summary}
