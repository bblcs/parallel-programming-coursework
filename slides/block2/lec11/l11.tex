...
\newcommand{\thisLectureNum}{\langMMNum}


\title[]{Lecture \thisLectureNum: \langMMTopic}
\subtitle[]{\langMMKey}
...
We studied this
\begin{itemize}
  \item \texttt{Lock}, code locking, data locking, lock splitting (Lecture~\basicNum)
  \item \texttt{Executor, Backoff, Condition} (Lecture~\basicTwoNum)
  \item \texttt{Monitor, CountDownLatch, ThreadLocal}, replication (Lecture~~\syncPrimitivesNum)
  \item Concurrent state machine, cancellation, interruption (Lecture~\patternsNum)
\end{itemize}
...
\questiontime{Word tearing. 

\begin{quote}
For the purposes of the Java programming language memory model, a single write to a non-volatile long or double value is treated as two separate writes: one to each 32-bit half. This can result in a situation where a thread sees the first 32 bits of a 64-bit value from one write, and the second 32 bits from another write.
\end{quote}

\url{https://docs.oracle.com/javase/specs/jls/se11/html/jls-17.html#jls-17.6}
}
...
\begin{itemize}
  ...
  \item \texttt{memory\_order\_acq\_rel} in C/C++ is \textbf{different} from Java \texttt{volatile}
...
\begin{itemize}
  \pause \item use \texttt{volatile} only when it simplifies your synchronization policy
