...
let gen_plot_op ~prefix (op : operation_type) (benches : benchmark list) =
  ...
  if !plot_commands <> [] then begin
    ...
    Printf.fprintf gp_oc "set terminal pngcairo size 800,600\n";
...
let gen_plot_ctype (ct : counter_type) (benches : benchmark list) =
  let ct_str = string_of_ctype ct in
  let dat_file = ct_str ^ ".dat" in
  let gp_file = ct_str ^ ".gp" in
  let oc = open_out dat_file in
  let plot_commands = ref [] in
...
let () =
  ...
  List.iter (fun ct -> gen_plot_ctype ct data) valid_ctypes;
